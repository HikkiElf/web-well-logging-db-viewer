...
const getSelectedAreaId = (areaName) => {
    selectedAreaId.value = refAreasInProject.value.find(
...
watch(toRef(props, "selectedProjectId"), async (newIndex) => {
    responseGetAreas.value = await axios.get(
        `https://well-logging.mrsmori.moe/areas?project_id=${props.selectedProjectId}`
    );
...
<template>
    <select name="Select Area" id="select-area" :selectedProjectId="props.selectedProjectId" v-model="selectedAreaName"
        @change="getSelectedAreaId(selectedAreaName)">
