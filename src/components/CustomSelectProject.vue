...
onMounted(async () => {
    try {
        responseGetProjects.value = await axios.get(
            `https://well-logging.mrsmori.moe/login?login=${loginPassword.login}&password=${loginPassword.password}`,
    ...
    refAllProjects.value = JSON.stringify(
        responseGetProjects.value.data.projects
    );
...
const getSelectedProjectId = (projectName) => {
    selectedProjectId.value = refAllProjects.value.find(
...
<template>
    <select name="Select Project" id="select-project" v-model="selectedProjectName"
        @change="getSelectedProjectId(selectedProjectName)">
